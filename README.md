...
- **AWS_ACCOUNT --> awsaccount**: Required environment variable that determines AWS account to be used with the stack. Needed for Lambda@Edge to work. Account id is a string of numbers, which can be gotten from AWS console top-right (click user name -> Account ID).
