...
interface LockResult {
  ...
  success: boolean;
  error?: string;
  skipped?: boolean; // True if balise was already locked or not found
}
...
async function executeLock(baliseId: number, userUid: string): Promise<LockResult> {
  try {
...
async function processBulkLock(baliseIds: number[], userUid: string): Promise<BulkLockResponse> {
  ...
  for (let i = 0; i < baliseIds.length; i += CONCURRENCY_LIMIT) {
    ...
    const chunkResults = await Promise.all(chunk.map((baliseId) => executeLock(baliseId, userUid)));
...
export async function handleRequest(event: ALBEvent): Promise<ALBResult> {
  try {
    ...
    const response = await processBulkLock(request.baliseIds, user.uid);
