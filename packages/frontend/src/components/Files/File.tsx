...
const label = { inputProps: { 'aria-label': 'select file' } };

export const NodeItem = ({ node, row, onFileClick = () => {}, isSelected }: NodeItemProps) => {
  const { entry } = node;
    ...
    >
      ...
      {isEditOpen && (
        <Grid item mobile={1} tablet={1} desktop={1}>
          <Checkbox {...label} checked={isSelected}></Checkbox>
        </Grid>
      )}
    </Grid>
  );
